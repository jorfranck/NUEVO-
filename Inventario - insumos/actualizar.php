<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $codigo = filter_var($_POST['id'], FILTER_VALIDATE_INT);

        if ($codigo) {
            // ELIMINAR EL EQUIPO
            $query = "DELETE FROM equipos WHERE codigo = ?";
                ...
                <tr>
                    ...
                    <td>
                        <form method="POST" class="w-100" onsubmit="return confirmaEliminacion();">
                            <input type="hidden" name="id" value="<?php echo $equipo['codigo'] ?>">
                            <input type="submit" class="boton-opc1" value="ELIMINAR">
                        </form>
                        <a href="/actualizar-ins.php?codigo=<?php echo $equipo['codigo']?>" 
